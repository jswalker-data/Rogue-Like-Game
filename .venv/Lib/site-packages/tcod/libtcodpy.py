...
def heightmap_rain_erosion(
    ...
    Args:
        ...
        sedimentationCoef (float): Amount of ground deposited when the drops
                                   stops to flow.
        rnd (Random | None): A tcod.Random instance, or None.
    """
...
def heightmap_add_voronoi(
    ...
    Args:
        ...
        rnd (Random | None): A Random instance, or None.
    """
    nbPoints = len(coef)
...
@pending_deprecate()
def namegen_parse(
    filename: str, random: tcod.random.Random | None = None
) -> None:
...
def noise_new(
    ...
    Args:
        ...
        random (Random | None): A Random instance, or None.
...
def random_set_distribution(
    rnd: tcod.random.Random | None, dist: int
) -> None:
    """Change the distribution mode of a random number generator.

    Args:
        rnd (Random | None): A Random instance, or None to use the default.
...
def random_get_float(
    rnd: tcod.random.Random | None, mi: float, ma: float
) -> float:
...
def random_get_int_mean(
    rnd: tcod.random.Random | None, mi: int, ma: int, mean: int
) -> int:
...
def random_get_float_mean(
    rnd: tcod.random.Random | None, mi: float, ma: float, mean: float
) -> float:
...
def random_restore(
    rnd: tcod.random.Random | None, backup: tcod.random.Random
) -> None:
...
def sys_save_screenshot(name: str | None = None) -> None:
    ...
    Args:
        file AnyStr | None: File path to save screenshot.
...
def sys_check_for_event(
    mask: int, k: Key | None, m: Mouse | None
) -> int:
    """Check for and return an event.

    Args:
        mask (int): :any:`Event types` to wait for.
        k (Key | None): A tcod.Key instance which might be updated with
                           an event.  Can be None.
        m (Mouse | None): A tcod.Mouse instance which might be updated
...
def sys_wait_for_event(
    mask: int, k: Key | None, m: Mouse | None, flush: bool
) -> int:
